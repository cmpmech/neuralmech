...
//   FORMULATION_ACOUSTIC  acoustic wave equation; default is the scalar wave eq.
//
// Scalar wave eq (gamma scales the background density rho0, single material field):
//   gamma rho0 u_tt - div(gamma rho0 c0^2 grad u) = f          (Herrmann et al. Eq. 3)
// Acoustic wave eq (transient acoustic topology optimization, gamma interpolates
// the inverse density rho^-1 and inverse bulk modulus kappa^-1):
//   kappa^-1 u_tt + d u_t - div(rho^-1 grad u) = f             (Eq. 16, + damping)
// Both use a one-cell ghost layer for the homogeneous Neumann BC. Memory layout is
// row-major with the fastest (last) axis padded to a multiple of 32; strides
// s0..s_{NDIM-2} are passed in, the last axis has unit stride.
...
// per-axis conservative flux: factor * [ g_plus (u_+ - u_c) - g_minus (u_c - u_-) ]
// scalar:   g = mat_face / (mat_c + mat_face)   (mat = gamma; folds the 1/(rho0 gamma_c) term)
// acoustic: g = 1 / (mat_c + mat_face)          (mat = density rho; cf. Eq. 31)
...
__global__ void fd_kernel(const real_t* __restrict__ u0,
                          ...
                          const real_t* __restrict__ damping,
                          const real_t dt,
#else
...
// (corners are left untouched, matching the 2D reference scheme).

__global__ void bc_kernel(real_t* __restrict__ u, const int axis,
                          const int N0
#if NDIM >= 2
...
// -----------------------------------------------------------------------
// sources/sensors address the grid through precomputed linear indices, so these
// kernels are dimension-agnostic.
//   scalar:   u += dt^2 / (rho0 gamma_c) * source        (Eq. 30 source term)
//   acoustic: u += kappa_c dt^2 * source                 (Eq. 31 source term)

__global__ void excitation_kernel(real_t* __restrict__ u, const real_t* __restrict__ source,
                                  const int* __restrict__ lin_index, const int num_sources,
                                  const real_t dt2, const real_t* __restrict__ scale){
...
__global__ void get_signal_kernel(const real_t* __restrict__ u, real_t* __restrict__ um,
                                  const int* __restrict__ lin_index, const int num_sensors){
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx < num_sensors){
