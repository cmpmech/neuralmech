...
# pure-python ctypes bindings to MKL pardiso/ilu/spmv, not shipped in the mlhp wheel
# extended here: pardisoFactorize also accepts scipy matrices (real or complex) and
# exposes refactorize() for repeated solves on a fixed sparsity pattern
...
class pardisoFactorize:
    def __init__(self, matrix, *, messageLevel=0, symmetric=None, symmetricHalf=None, mklLibrary=None):
    ...
    def _setMatrix(self, matrix, symmetricHalf=None):
        if hasattr(matrix, "data_address"): # mlhp sparse matrix (real double)
            ...
            self.symmetricHalf = matrix.symmetricHalf if symmetricHalf is None else symmetricHalf
            self.addresses = (matrix.data_address, matrix.indptr_address, matrix.indices_address)
            ...
            self.matrix = (numpy.array(csr.data, dtype=dtype),
    ...
    def __call__(self, rhs, target=None):
        if isinstance(rhs, numpy.ndarray):
            ...
            x = numpy.empty(b.shape, dtype=dtype, order="F") if target is None else target
            self._callPardiso(33, b.ctypes.data, x.ctypes.data, 1 if b.ndim == 1 else b.shape[1])
            return x
...
if __name__ == "__main__":
    ...
    S = (B1 + B1.T + 10.0 * scipy.sparse.eye(n)) + 1j * (B2 + B2.T + scipy.sparse.eye(n))
