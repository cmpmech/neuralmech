...
class MLP:
    def __init__(self, layers, activation, grad_activation):
        ...
        self.biases = [np.random.randn(layers[i + 1]) for i in
                       range(self.L)]
...
# --------------------------- backpropagation ----------------------------
    def backward(self, grad_cost):
        ...
        if self.L == 1: # input layer has no activation
            ...
            self.grad_weights[-1] = (self.act(self.preacts[-2]).T@deltal /
                                     samples)
        self.grad_biases[-1] = np.mean(deltal, 0)

        for l in range(self.L - 2, -1, -1):
            deltal = (deltal@self.weights[l + 1].T *
                      self.grad_act(self.preacts[l + 1]))
            if l == 0: # input layer has no activation
                self.grad_weights[l] = self.preacts[0].T@deltal / samples
            else:
                self.grad_weights[l] = (self.act(self.preacts[l]).T@deltal
                                        / samples)
