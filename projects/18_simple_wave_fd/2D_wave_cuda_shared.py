...
# implementation
DTYPE = cp.float32
THREADS_J, THREADS_I = 128, 4  # 16, 16 performs similarly here
...
if PRECOMPILED:
    ...
    fd_kernel = module.get_function("fd_kernelV4")
else:
    source = (BASE_DIR / "step2D_wave.cu").read_text()
    fd_kernel = cp.RawKernel(source, "fd_kernelV4", COMPILER_OPTIONS)
...
# -------------------------------- book postprocessing --------------------------------
    ...
    plt.savefig(RGB_PDF_DIR / "wave_shared.pdf")
