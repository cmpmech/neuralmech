...
if PRECOMPILED:
    ...
    fd_kernel = module.get_function("fd_kernelV2")
else:
    source = (BASE_DIR / "step2D_wave.cu").read_text()
    fd_kernel = cp.RawKernel(source, "fd_kernelV2", COMPILER_OPTIONS)

blocks_j = (NY + THREADS_J - 1) // THREADS_J
blocks_i = (NX + THREADS_I - 1) // THREADS_I
...
def fd_step(u0, u1, u2):
    fd_kernel(
        (blocks_j, blocks_i),
        (THREADS_J, THREADS_I),
        (u0, u1, u2, DTYPE(WAVESPEED), DTYPE(dt), DTYPE(dx), DTYPE(dy), NX, NY),
    )
    return u2
...
# -------------------------------- book postprocessing --------------------------------
    ...
    plt.savefig(RGB_PDF_DIR / "wave_coalesced.pdf")
