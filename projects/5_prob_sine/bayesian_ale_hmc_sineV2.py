...
# ablation of the gamma trick: instead of fitting a gamma likelihood to the squared
# residual r = (mu(x) - y)^2, the variance network is fitted with the plain gaussian nll
# on r -- the textbook two-step mean-variance estimate, whose gradient still carries the
# 1/s^4 term. each r is a single chi-squared draw, so the fit chases individual small
# residuals and the variance collapses well below the true noise floor; the resulting
# large 1/s^2 stiffens the posterior and the hmc acceptance rate drops from 0.86 to 0
def gaussian_nll(y_pred, r):
    var = aleatoric_variance(y_pred)
    nll = 0.5 * torch.log(2 * torch.pi * var) + r / (2 * var)
...
def aleatoric_variance(y_pred):
    return F.softplus(y_pred) + 1e-6  # keep the 1/s^2 terms finite


# model settings
LAYERS = [1, 64, 64, 1]
VAR_LAYERS = [1, 16, 1]  # aleatoric variance
...
var_cost = train(
    var_model,
    lambda: gaussian_nll(var_model(x_train), residual),
    VAR_EPOCHS,
    VAR_REGULARIZATION,
)
toc = time.time()
...
# -------------------------------- book postprocessing --------------------------------
    ...
    save_csv(
        CSV_DIR / "ale_hmc_gaussian_train.csv",
