...
for epoch in pbar:
    ...
    for x, y in train_loader:
        ...
        optimizer.zero_grad()
        y_pred = model(x)
        cost = cost_fun(y_pred, log_var, y)
        cost.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for x, y in val_loader:
            x, y = standardizex(x.to(device)), standardizey(y.to(device))
            y_pred = model(x)
            val_cost[epoch] += cost_fun(y_pred, log_var, y).item()
...
with torch.no_grad():
    ...
    mean_test = standardizey.inverse(y_pred_test).cpu().numpy()
    var_test = torch.exp(log_var).cpu().numpy() * (standardizey.x_std.item() ** 2)
    std_test = np.sqrt(var_test)

print(f"recovered std: {std_test.item():.4f}")
