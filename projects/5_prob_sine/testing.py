...
# define loss
def elbo(model, x, y, prior_std, mc_samples, kl_weight):
    nll = 0
    for _ in range(mc_samples):
        ...
        nlls = 0.5 * torch.log(2 * torch.pi * var) + (y - mean) ** 2 / (2 * var)
        nll += torch.mean(nlls)
...
# model settings
LAYERS = [1, 32, 32, 32, 2]  # second output predicts the input-dependent log-variance
...
# ground truth
mean = target(x_test).cpu()
std = noise_std(x_test).cpu()

fig, ax = plt.subplots()
ax.plot(x_test.cpu(), target(x_test).cpu(), "k")
ax.fill_between(
    x_test.squeeze().cpu(),
