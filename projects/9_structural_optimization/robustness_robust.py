...
# postprocessing
NAME = "robustness_robust"
SPL_RANGE = (50.0, 110.0)  # shared color scale in dB, 60 dB dynamic range
THRESHOLD = 0.5
ETA_PERTURBED = 0.4  # uniform dilation of the final design; 0.6 erodes
...
BETA_GROWTH = 1.02
RMIN = 6  # in design voxels, large enough for a threshold shift to move the edges
...
for it in pbar:
    ...
    worst = [
        sensitivity(physical(x_tilde, beta, eta), x_tilde, beta, eta) for eta in ETAS
    ]
    costs = [phi for phi, _ in worst]
    phi, grad = worst[int(np.argmax(costs))]
    phi0 = phi if phi0 is None else phi0
    optimizer.zero_grad()
    ...
    pbar.set_postfix(
        {
            "Lp": " ".join(f"{10.0 * np.log10(c / P0**2):.1f}" for c in costs),
            "beta": f"{beta:.0f}",
        }
    )
...
x_tilde = density_filter(expand(x.detach().numpy()))
designs = {
    "": (physical(x_tilde, beta, ETA_I) > THRESHOLD) * band,
    "_perturbed": (physical(x_tilde, beta, ETA_PERTURBED) > THRESHOLD) * band,
}

sound_level = lambda phi: 10.0 * np.log10(phi / P0**2)
print(f"no ceiling L_p {sound_level(objective(np.zeros((NX, NY)))[0]):.1f} dB")

postmesh = mlhp.domainCellMesh(domain, [DEGREE + 1] * 2)
pressure_cmap = cmr.get_sub_cmap(cmr.fusion_r, 0.5, 1.0)
for suffix, rho in designs.items():
    phi, p, _ = objective(rho.astype(float))
    print(
        f"{NAME + suffix:<{len(NAME) + 10}}  cost {phi:.3e}  L_p {sound_level(phi):.1f} dB  "
        f"vol {rho.sum() / n_band:.3f}"
    )
    ...
    mlhp.basisOutput(
        ...
        cellmesh=postmesh,
        processors=[
            mlhp.solutionProcessor(2, mlhp.DoubleVector(p.real.tolist()), "Re"),
            mlhp.solutionProcessor(2, mlhp.DoubleVector(p.imag.tolist()), "Im"),
        ],
        output=pressure,
    )
    ...
    ax.tricontourf(
        ...
        spl,
        levels=np.linspace(*SPL_RANGE, 64),
        cmap=pressure_cmap,
        extend="both",
    )
    ax.imshow(
        np.ma.masked_where(rho.T < 0.5, rho.T),
        origin="lower",
    ...
    if args.book:
        plt.savefig(RGB_PDF_DIR / f"{NAME + suffix}.pdf")
        plt.close()
    else:
        plt.show()
