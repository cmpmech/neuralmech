...
# -------------------------- load trained model --------------------------
seq_len = 128
model = torch.load(f'../../models/ae_3dof_{seq_len}.pt2', weights_only=False).to(device)
model.eval()
...
def process_with_model(x):
    ...
    x_pred = standardizex.inverse(model(standardizex(x_in)))
...
# --------------------------------- AUC ----------------------------------
thresholds = np.logspace(-7, -3, 100)
