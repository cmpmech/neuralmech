...
# ---------------------------- load map data -----------------------------
gdf = gpd.read_file("../../data/ne_50m_admin_0_countries.shp")
...
grid = rasterize([(geom, 1) for geom in ghana.geometry],
...
# p: use segments, q25: minimum angle 25 deg, a %d: max area
tri_output = tr.triangulate(tri_input, 'pq25a0.0004')  # fine mesh
# tri_output = tr.triangulate(tri_input, 'pq25a0.0002')  # very fine mesh

# ---------------------------- postprocessing ----------------------------
# visualize the mesh
fig, ax = plt.subplots(figsize=(10, 10), dpi=100)
ax.triplot(tri_output['vertices'][:, 0], tri_output['vertices'][:, 1],
           tri_output['triangles'], 'b-', linewidth=0.5)
...
for tri in triangles:
    ...
    edges.extend([[tri[1], tri[0]], [tri[2], tri[1]], [tri[0], tri[2]]])  # bidirectional
...
torch.save({
    ...
    'pos': pos,                # [nodes, 2]
    'triangles': torch.tensor(triangles, dtype=torch.long)  # optional for visualization
}, '../../data/ghana_mesh.pt')