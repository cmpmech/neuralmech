...
Steps:
  ...
  5. Moment accumulation offloaded to CUDA kernel    (GPU).
"""
...
# ---------------------------------------------------------------------------
# Load CUDA kernel (once at module import)
# ---------------------------------------------------------------------------

_KERNEL_PATH = Path(__file__).parent / "momentfitting.cu"
_cuda_module = cp.RawModule(code=open(_KERNEL_PATH).read())
_kernel      = _cuda_module.get_function("accumulate_moments")
...
# ---------------------------------------------------------------------------
# Core: compute moments for one element (GPU accumulation)
# ---------------------------------------------------------------------------

def compute_moments(
    nvoxels: tuple,
    polynomial_degree: int,
    alpha_func,           # callable: (xyz: np.ndarray) -> float  — stays on CPU
) -> tuple:
    """
    ...
    polynomial_degree : p — Lagrange basis order 2p+1, stride = 2p+1 nodes
    alpha_func        : material/indicator function, called on reference coords
    ...
    """
    D         = len(nvoxels)
    stride    = 2 * polynomial_degree + 1   # == lag_order + 1
    lag_order = stride - 1
    ...
    # ---- 3. Lagrange evaluation (CPU) ---------------------------------------
    # lag_flat[d] : shape (n_vox_d * stride,) — row-major L[j, i]
    lag_flat = []
    n_vox = []
    for d in range(D):
        L = lagrange_eval(elem_pts_per_axis[d], voxel_pts_per_axis[d])
        lag_flat.append(L.ravel().astype(np.float64))   # (n_vox_d * stride,)
        n_vox.append(len(voxel_pts_per_axis[d]))




    # Concatenate per-axis arrays; kernel uses per-axis offsets (sum of n_vox[k]*stride for k<d)
    lag_concat = np.concatenate(lag_flat)   # 1-D

    # ---- 4. alpha_w: evaluate alpha at every voxel IP (tensor product) ------
    n_vox_total = int(np.prod(n_vox))
    alpha_w = np.empty(n_vox_total, dtype=np.float64)

    for flat_ip, ijk in enumerate(
        itertools.product(*[range(n) for n in n_vox])
    ):
        rst = np.array([voxel_pts_per_axis[d][ijk[d]] for d in range(D)])
        w   = np.prod([voxel_wts_per_axis[d][ijk[d]] for d in range(D)])
        alpha_w[flat_ip] = alpha_func(rst) * w * voxel_detJ

    # ---- 5. GPU accumulation ------------------------------------------------
    d_lag   = cp.asarray(lag_concat)
    d_aw    = cp.asarray(alpha_w)
    d_mom   = cp.zeros(stride ** D, dtype=cp.float64)

    n_moments  = stride ** D
    block_size = 128
    grid_size  = (n_moments + block_size - 1) // block_size
    ...
    _kernel(
        ...
        (
            ...
            np.int32(stride),
            np.int32(n_vox[0]),
            np.int32(n_vox[1] if D >= 2 else 1),
            np.int32(n_vox[2] if D >= 3 else 1),
        )
    )

    moments = cp.asnumpy(d_mom)
