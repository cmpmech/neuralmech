...
n = matrix.shape[0]
K = sp.csr_matrix(
    (matrix.data_array, matrix.indices_array, matrix.indptr_array), shape=(n, n)
)
rhs = np.array(vector.array)

tic = time.time()
u_int = pypardiso.spsolve(K, rhs)
print(f"Pardiso: {time.time() - tic:.2f}s")

all_dofs = mlhp.inflateDofs(mlhp.DoubleVector(u_int.tolist()), dirichlet)
...
out = str(RESULTS_DIR / f"elastic_mlhp_momentfitting_direct_{ct_file[:-4]}")
