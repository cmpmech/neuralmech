...
from solvers.wave_sensitivity_pml import build_sponge, compute_sensitivity_pml

BASE_DIR = Path(__file__).parent
RESULTS_DIR = (BASE_DIR / "../../results").resolve()
ANIMATION_DIR = RESULTS_DIR / "animations/animation_frames/topopt_acoustic2D_pml"
...
# Herrmann et al. 2026, Fig. 2) with absorbing top and bottom edges. A sine-burst point
# source on the left wall sends a wave down an open central channel towards a target box
# on the right wall; aluminium is distributed in the upper and lower blocks FLANKING the
# channel to trap and divert the wave before it reaches the target. The top and bottom
# edges carry an absorbing sponge layer so the diverted energy escapes instead of
# reverberating in a closed room. The interior stays lossless, so the boundary-
# reconstruction adjoint (wave_sensitivity_pml) computes an exact gradient there.
...
FREQUENCY = (
    200  # 70.0  # source centre frequency [Hz] # TODO FIGURE OUT WHAT MAXIMUM IS
)
CYCLES = 3
AMPLITUDE = 1e3
T = 0.18  # simulated time window [s]

# absorbing sponge on the top and bottom edges (kept out of the design region)
SPONGE_WIDTH = 12  # strip thickness [cells]
SPONGE_BETA = 1.5  # peak update damping coefficient beta = 0.5 kappa d dt at the edge
# (tuned at this resolution: retains ~5% of peak pulse energy; higher beta reflects off
# the strip interface, a wider+gentler strip absorbs more but shrinks the design region)
...
def simulate_frames(gamma, record_every):
    # forward wavefield through the absorbing domain, snapshots for the animation
    mat = build_materials(sim, gamma, sponge)
...
# absorbing sponge on the top and bottom edges (air bulk modulus sets the scale)
sponge = build_sponge(
    sim,
    SPONGE_WIDTH,
    2.0 * SPONGE_BETA / (KAPPA1 * dt),
    power=3,
    sides=[(1, "lo"), (1, "hi")],
)
...
# design region: upper and lower blocks flanking the open central channel, inset from
# the top and bottom edges so the design never overlaps the absorbing sponge
...
y_lo = 1 + SPONGE_WIDTH
y_hi = Nx[1] - 1 - SPONGE_WIDTH
DESIGN[x0 : Nx[0] - 1, y_lo:ch_lo] = 1.0  # lower block
DESIGN[x0 : Nx[0] - 1, ch_hi:y_hi] = 1.0  # upper block
...
if args.animate:
    ...
    ax.axis("off")
    fig.subplots_adjust(left=0, right=1, top=1, bottom=0)
    fig.savefig(RESULTS_DIR / "topopt_acoustic2D_pml.png")
    plt.close()
else:
