...
ANIMATION_DIR = RESULTS_DIR / "animations/animation_frames/topopt_heatV2"
...
# ------------------------------------ optimization -----------------------------------
# V2 reference: the original continuous mmapy MMA loop -- move-limited steps with the
# moving asymptotes kept across the whole run. Same robust three-field formulation as
# topopt_heat.py; only the optimizer differs (mmapy here vs staged nlopt there), so the
# two files isolate the optimizer's effect on the design and the iteration count
n = NX * NY
...
for it in pbar:
    ...
    # objective: compliance of the eroded design
    u = np.zeros(ndof)
    ...
    # volume constraint on the dilated design; bound rescaled so the intermediate hits VOLFRAC
    vfrac_d *= VOLFRAC / x_i.mean()
    dvol = filter_adjoint(dprojection(x_tilde, beta, ETA_D) / n)

    f0val = compliance / c_ref
    df0dx = (dc.ravel() / c_ref).reshape(n, 1)
    fval = np.array([[x_d.mean() / vfrac_d - 1.0]])
    dfdx = (dvol.ravel() / vfrac_d).reshape(1, n)
...
if args.book:
    ...
    plt.savefig(
        RESULTS_DIR / "topopt_heatV2_temp.png", bbox_inches="tight", pad_inches=0
    )
    plt.close()
...
for field, name in ((x_int, "topopt_heatV2"), (rho_thresh, "topopt_heatV2_thresh")):
    fig, ax = plt.subplots(figsize=(NX / 100, NY / 100), dpi=150)
