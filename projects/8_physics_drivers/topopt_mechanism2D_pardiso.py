...
from solvers.optimization import MMA, DensityFilter, PardisoStructuredFEM
...
for it in pbar:
    ...
    # state and adjoint share K(rho) + springs: one factorization, two rhs columns
    u = np.zeros(ndof)
    lam = np.zeros(ndof)
    ...
    vol = rho.mean()
    dvol = density_filter.adjoint(np.full((NX, NY), 1.0 / n))
