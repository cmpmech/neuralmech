...
for it in pbar:
    ...
    pbar.set_postfix(
        c=f"{compliance:.2e}", vol=f"{mean_rho:.3f}", m=f"{weight:.1e}"
    )
...
for ax, field in zip(axes, (rho, rho_thresh)):
    ...
    ax.axis("off")
fig.suptitle(
    f"ae [{DUMP_TAG}]  c {compliance_init:.3e} -> {compliance:.3e}  "
...
plt.savefig(DUMP_DIR / f"topopt_latent_ae_{DUMP_TAG}.png")
plt.close()
