...
# -------------------------- load trained model --------------------------
model = torch.load(
    BASE_DIR / f"../../models/shape_ae_{domain_size}.pt2",
    weights_only=False,
    map_location=device,
)
model.eval()
...
for i in range(samplesx):
    for j in range(samplesy):
        ax[j, i].imshow(gen_shapes[i, j].T.cpu(), cmap="binary", origin="lower")
...
for label, latent in zip(labels, latents):
    save_csv(
        BASE_DIR / f"../../results/data/shapes_ae_latent_{label}.csv",
        x=latent[:, 0],
        y=latent[:, 1],
    )
...
for i in range(samplesx):
    for j in range(samplesy):
        ...
        fig.tight_layout(pad=0)
        plt.savefig(
            BASE_DIR / f"../../results/rgb_pdf/genshapes_ae_{i}{j}_{box}.pdf",
            bbox_inches="tight",
            pad_inches=0,
        )
        plt.close()
