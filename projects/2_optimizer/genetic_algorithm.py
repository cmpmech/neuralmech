...
def evolve():
    ...
    for _ in range(G):
        ...
        ids = np.argsort(f(population.T))
        parents = population[ids[:ELITE]]
...
for k, P in enumerate(history):
    ...
    ax.plot(P[:, 0], P[:, 1], "o", ms=4, color=color)
...
# -------------------------------- book postprocessing --------------------------------
    ...
    fig.savefig(RGB_PDF_DIR / "ga.pdf")
    save_csv(CSV_DIR / "ga_history.csv", x=np.arange(0, G + 1), y=cost_history)
plt.close("all")
