...
activations = [torch.nn.ReLU(inplace=True), torch.nn.Identity(), torch.nn.ReLU(inplace=True)]
...
with torch.no_grad():
    ...
    model.model[6].bias.data.fill_(1.)
    for i in range(1, neurons):
        if i % 2 == 0:
...
# ------------------------- book postprocessing --------------------------
save_csv(f'../../results/universal_approx_depth_comb.csv', x=x_test.squeeze(), ypred=y_pred.squeeze())